# ⚡ Tùng Lâm Automatic — Robotics • Embedded • Control • Vision

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=230&text=T%C3%B9ng%20L%C3%A2m%20Automatic&fontAlign=50&fontAlignY=35&desc=Build%20robots%20that%20run%20smooth%20and%20stay%20stable&descAlign=50&descAlignY=58&color=gradient" />
</p>

<p align="center">
  <a href="https://github.com/Tunglam0605">
    <img src="https://img.shields.io/badge/GitHub-Tunglam0605-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <img src="https://img.shields.io/badge/Robotics-Mecanum%20Base%20%7C%20Robot%20Arm-0B84F3?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Embedded-STM32%20%7C%20ESP32%20%7C%20DMA-10B981?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Vision-OpenCV%20%7C%20YOLOv8-7C3AED?style=for-the-badge" />
</p>

<p align="center">
  Xin chào 👋 Mình là <b>Nguyễn Khắc Tùng Lâm</b> — <b>Tùng Lâm Automatic</b><br/>
  Mình làm robot theo style: <b>chạy mượt</b> • <b>đi đúng</b> • <b>debug nhanh</b> • <b>an toàn</b><br/>
  Câu cửa miệng: <i>Robot không sai — chỉ là mình chưa log đủ</i> 😄
</p>

---

## 🚀 Nổi bật trong một câu

- 🤖 Robot thi đấu và robot nghiên cứu: mecanum, cánh tay, tích hợp hệ thống  
- 🧠 Điều khiển chuyển động: PID, Fuzzy PID, encoder feedback, profile vận tốc  
- 🧩 Firmware: STM32 F1 F4, UART, USB CDC, DMA, timers, lưu cấu hình Flash  
- 👁️ Thị giác máy: OpenCV, YOLOv8, ROI calibration, mapping pixel sang mm  
- 🧰 Hệ sinh thái: ROS2, Python GUI PySide6, Git workflow

---

## 🧰 Tech stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=c,cpp,python,linux,git,github,vscode,qt,ros,opencv&perline=10" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/STM32-HAL%20%7C%20DMA%20%7C%20Timers-03234B?style=for-the-badge&logo=stmicroelectronics&logoColor=white" />
  <img src="https://img.shields.io/badge/ROS2-Navigation%20%7C%20Control-22314E?style=for-the-badge&logo=ros&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-GUI%20%7C%20Tools-3776AB?style=for-the-badge&logo=python&logoColor=white" />
</p>

---

## 🧭 Mình đang build gì

### 🛞 Mobile robot mecanum
- Kinematics: IK và FK cho 4 bánh
- Encoder feedback, clamp vận tốc, timeout an toàn
- Hướng tới: odometry và navigation với ROS2

### 🦾 Robot arm pick and place
- IK, safe sequence, limit switch, phối hợp base và arm
- Tối ưu độ ổn định khi nâng hạ và gắp thả

### 👁️ Vision module cho auto pick
- Detect object với YOLO và xử lý ảnh với OpenCV
- Calib ROI, map pixel sang tọa độ mm để pick chuẩn

---

## 🌟 Projects tiêu biểu

| Project | Mô tả nhanh | Link |
|---|---|---|
| TungLamvsAGV-F407 | Node STM32F407 cho robot base: encoder, lệnh vận tốc, điều khiển, config Flash | https://github.com/Tunglam0605/TungLamvsAGV-F407 |
| TunglamvsRobot2025 | Hệ thống robot tổng: base mecanum, arm, GUI điều khiển, workflow test thi đấu | https://github.com/Tunglam0605/TunglamvsRobot2025 |
| TungLamvsRos2 | ROS2 workspace: navigation, controller tuning, integration robot | https://github.com/Tunglam0605/TungLamvsRos2 |

---


## 🏆 Thành tích và hoạt động

- 🏆 **Vô địch Robot Gắp Vật 2025** — do **CLB Robot & Công nghệ**, **Khoa Điện – Tự động hoá (UNETI)** tổ chức
- 🥇 RoboBall Challenge — Giải Nhất
- 🤖 Robocon, robot đánh golf, robot ném bóng — nhiều dự án và giải pháp sáng tạo
- 👨‍🏫 Mentor và hỗ trợ đội nhóm, CLB robot và công nghệ

---

## 📬 Liên hệ

- Email: tunglam652004@gmail.com
- Số điện thoại: 0325270213
- GitHub: https://github.com/Tunglam0605

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=130&section=footer&color=gradient" />
</p>
